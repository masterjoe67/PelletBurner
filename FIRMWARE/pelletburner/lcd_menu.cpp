...
static void lcd_date_time_menu();
...
{
	if (stato_funzionamento == service) {
		lcd_return_from_service();
		return;
	}
...
static void lcd_parameter_menu()
{
	...
	MENU_ITEM(submenu, MSG_DATETIME, lcd_date_time_menu);
	MENU_ITEM(function, MSG_STORE_EPROM, Config_StoreSettings);
	MENU_ITEM(function, MSG_LOAD_EPROM, Config_RetrieveSettings);
	MENU_ITEM(function, MSG_RESTORE_FAILSAFE, Config_ResetDefault);
    END_MENU();
...
static void lcd_date_time_menu() {
	...
	MENU_ITEM_EDIT(int3, MSG_HOUR, &ore, 0, 24, PSTR(MSG_HOUR), nullptr);
	MENU_ITEM_EDIT(int3, MSG_MINUTS, &minuti, 0, 59, PSTR(MSG_MINUTS), nullptr);
	END_MENU();
}
...
static void lcd_termostat_menu() {
	...
	MENU_ITEM_EDIT(int3, MSG_PWR_LOW, &_Th43, 0, 850, PSTR(HELP_PWR_LOW_MOD), PSTR(RANGE_0_250_C));
...
void lcd_update()
{
    ...
    if (lcd_next_update_millis < millis())
    {
        ...
        if(timeoutToStatus < millis() && currentMenu != lcd_status_screen && stato_funzionamento != service)
        {
